...
def press(num):
    ...
    if num=='+' or num=='-' or num=='*' or num=='/':
        ...
        equation.set(equation.get() + str(num))                # update the expression by using set method
...
def cls(): # class which tell about the gui cal. postion and color
    global expression
    expression = ""
    equation.set("")
gui = Tk()
...
gui.geometry("270x150+650+300")

equation = StringVar()
expression_field = Entry(gui, textvariable=equation, state='disabled')
expression_field.grid(columnspan=4, ipadx=70,ipady=4)


button1 = Button(gui, text=' 2 ', fg='black', bg='light blue',
...
plus = Button(gui, text=' + ', fg='black', bg='light yellow', 
            command=lambda: press("+"), height=1, width=7)
plus.grid(row=2, column=3)

minus = Button(gui, text=' - ', fg='black', bg='light yellow',
            command=lambda: press("-"), height=1, width=7)
minus.grid(row=3, column=3)

multiply = Button(gui, text=' * ', fg='black', bg='light yellow',
                command=lambda: press("*"), height=1, width=7)
multiply.grid(row=4, column=3)

divide = Button(gui, text=' / ', fg='black', bg='light yellow',
                command=lambda: press("/"), height=1, width=7)
...
clear = Button(gui, text='Clear', fg='black', bg='light yellow',
            command=lambda:cls(), height=1, width=7)
clear.grid(row=6, column=1)

Decimal= Button(gui, text='.', fg='black', bg='light yellow',
                command=lambda: press('.'), height=1, width=7)
